...
export class UserClient {
    ...
    getUserWithPagination(userID: number, pageNumber: number, pageSize: number): Promise<PaginatedListOfUserBriefDto> {
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetUserWithPagination(_response);
        });
    }

    protected processGetUserWithPagination(response: Response): Promise<PaginatedListOfUserBriefDto> {
        followIfLoginRedirect(response);
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
